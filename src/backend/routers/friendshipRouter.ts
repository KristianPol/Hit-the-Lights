...
friendshipRouter.post('/request', (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!parsedRequesterId || !parsedAddresseeId) {
      unit.complete(false);
      res.status(400).json({ success: false, error: 'Both requesterId and addresseeId are required' });
      return;
    }
    ...
    if (result.success) {
      // If an initial message was provided, store it directly (bypasses friendship check)
...
friendshipRouter.post('/accept', (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!parsedFriendshipId || !parsedUserId) {
      unit.complete(false);
      res.status(400).json({ success: false, error: 'friendshipId and userId are required' });
      return;
    }
...
friendshipRouter.delete('/:userId/:friendId', (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!userId || !friendId) {
      unit.complete(false);
      res.status(400).json({ success: false, error: 'Invalid userId or friendId' });
      return;
    }
