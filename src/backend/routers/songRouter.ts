...
import SongServiceAsync from '../services/SongServiceAsync';
...
songRouter.post('/add', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (usePostgres) {
      const svc = new SongServiceAsync();
      const result = await svc.addSong({ name, author, bpm: parseInt(bpm, 10), length, songUrl, coverUrl, ownerId: parseOptionalNumber(ownerId) ?? null, isPublic: parseVisibility(isPublic, true) });
      if (result.success) {
...
songRouter.get('/all', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (usePostgres) {
      const svc = new SongServiceAsync();
      const songs = await svc.getAllSongs(viewerId);
...
songRouter.get('/count/:ownerId', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const viewerId = parseOptionalNumber(req.query['viewerId']);
    if (isNaN(ownerId) || ownerId <= 0) { if (unit) unit.complete(); res.status(400).json({ success: false, error: 'Invalid owner ID' }); return; }
    if (usePostgres) {
      const svc = new SongServiceAsync();
      const count = await svc.getUploadedSongCount(ownerId, viewerId);
...
songRouter.get('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const viewerId = parseOptionalNumber(req.query['viewerId']);
    if (isNaN(songId)) { if (unit) unit.complete(); res.status(400).json({ success: false, error: 'Invalid song ID' }); return; }

    if (usePostgres) {
      const svc = new SongServiceAsync();
      const song = await svc.getSongById(songId, viewerId);
...
songRouter.patch('/:id/visibility', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isNaN(songId)) { if (unit) unit.complete(false); res.status(400).json({ success: false, error: 'Invalid song ID' }); return; }
    const parsedOwnerId = parseOptionalNumber(ownerId);
    if (parsedOwnerId === undefined) { if (unit) unit.complete(false); res.status(400).json({ success: false, error: 'ownerId is required' }); return; }
    ...
    if (usePostgres) {
      ...
      if (result.success) res.status(200).json({ success: true, song: result.song, message: 'Song visibility updated' }); else res.status(result.error === 'Song not found' ? 404 : 403).json({ success: false, error: result.error });
    } else {
      const svc = new SongService(unit!);
...
songRouter.get('/:songId/difficulties', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (usePostgres) {
      const svc = new SongServiceAsync();
      const difficulties = await svc.getSongDifficulties(songId, viewerId);
      if (difficulties === undefined) { res.status(404).json({ success: false, error: 'Song not found or not accessible' }); return; }
...
songRouter.post('/:songId/difficulties', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (parsedDifficulty === undefined) { if (unit) unit.complete(false); res.status(400).json({ success: false, error: 'difficulty is required' }); return; }
    if (!Array.isArray(notes)) { if (unit) unit.complete(false); res.status(400).json({ success: false, error: 'notes must be an array' }); return; }

    if (usePostgres) {
      const svc = new SongServiceAsync();
      const result = await svc.addSongDifficulty(songId, parsedOwnerId, parsedDifficulty, notes);
      if (result.success) res.status(201).json({ success: true, difficulty: result.difficulty, message: 'Difficulty uploaded successfully' }); else res.status(result.error === 'Song not found' ? 404 : 400).json({ success: false, error: result.error });
    } else {
      const svc = new SongService(unit!);
...
songRouter.get('/:songId/difficulties/:difficultyId/leaderboard', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const viewerId = parseOptionalNumber(req.query['viewerId']);
    if (isNaN(songId) || isNaN(difficultyId)) { if (unit) unit.complete(); res.status(400).json({ success: false, error: 'Invalid song or difficulty ID' }); return; }
    if (usePostgres) {
      const svc = new SongServiceAsync();
      const result = await svc.getDifficultyLeaderboard(songId, difficultyId, viewerId);
      if (!result) { res.status(404).json({ success: false, error: 'Leaderboard not found or inaccessible' }); return; }
...
songRouter.post('/:songId/difficulties/:difficultyId/leaderboard', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isNaN(songId) || isNaN(difficultyId)) { if (unit) unit.complete(false); res.status(400).json({ success: false, error: 'Invalid song or difficulty ID' }); return; }
    const parsedUserId = parseOptionalNumber(userId);
    if (parsedUserId === undefined) { if (unit) unit.complete(false); res.status(400).json({ success: false, error: 'userId is required' }); return; }
    ...
    if (parsedScore === undefined || parsedMaxCombo === undefined || parsedAccuracy === undefined) { if (unit) unit.complete(false); res.status(400).json({ success: false, improved: false, error: 'score, maxCombo, and accuracy are required' }); return; }
    ...
    if (usePostgres) {
      ...
      if (result.success) res.status(200).json({ success: true, improved: result.improved, entry: result.entry, message: result.improved ? 'Highscore updated' : 'Score did not beat existing highscore' }); else { const status = result.error === 'Song not found or not accessible' ? 403 : result.error === 'Difficulty not found' ? 404 : 400; res.status(status).json({ success: false, improved: false, error: result.error }); }
    } else {
      const svc = new SongService(unit!);
...
songRouter.get('/:songId/difficulties/:difficultyId/chart', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (usePostgres) {
      ...
      if (!result) { res.status(404).json({ success: false, error: 'Chart not found or inaccessible' }); return; }
      res.status(200).json(result);
    } else {
      const svc = new SongService(unit!);
